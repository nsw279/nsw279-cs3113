//
//  Entity.h
//  NYUCodebase
//
//  Created by Noah Weitz on 4/16/19.
//  Copyright © 2019 Ivan Safrin. All rights reserved.
//

#ifndef Entity_h
#define Entity_h

#define TILE_SIZE 0.7f

#include "Graphical.h"

class Bullet;

class Entity {
protected:
    float height;
    float width;
    float xPos;
    float yPos;
    bool isPlayer;
    bool alive;
    bool active;
    
    SpriteSheet sprite;
    
    float xVel;
    float yVel;
    float xAccel;
    float yAccel;
    float gravity;
    float friction;
    
public:
    Entity(); // Constructors
    Entity(float h, float w, float xp, float yp, float xv, float yv, float xa, float ya, float grav);
    void setSprite(GLuint texid, float u, float v, float w, float h, float size);
    
    // Getter methods
    float getHeight();
    float getWidth();
    float getXPos();
    float getYPos();
    float getXVel();
    float getYVel();
    float getXAccel();
    float getYAccel();
    float getGravity();
    float lerp(float v0, float v1, float t);
    
    bool staticTrueDynamicFalse();
    bool getAlive();
    bool getActive();
    
    // Other Methods
    void setLocation(float newX, float newY);
    void move(float acc);
    void shoot(float elapsed, std::vector<Bullet*>& bullets, GLuint bTex);
    void setPlayer();
    void setEnemy();
    void setAlive(bool val);
    void setYVel(float val);
    void setYAccel(float val);
    
    // Stuff important to the game
    void update(float elapsedTime);
    void draw(ShaderProgram& program, glm::mat4& modelMatrix);
    SpriteSheet* getSprite();
    
    Bullet* myBullet;
};

class Enemy : public Entity {
public:
    bool whichType;
    bool kamiState;
    Bullet* eBullet;
    
    Enemy();
    Enemy(bool type);
    void eShoot();
    void goForIt(Entity* target, float elapsed);
};

class Barrier : public Entity {
public:
    Barrier();
    Barrier(float xp, float yp, float sz);
    bool isHit();
};


#endif /* Entity_h */
